...
void McpServer::AddCommonTools() {
    ...
    AddTool("self.audio_speaker.set_volume",
        ...
        PropertyList({
            Property("volume", kPropertyTypeInteger, 0, 150)
        }),
        [&board](const PropertyList& properties) -> ReturnValue {
    ...
    AddTool("self.ir.tv_b_gone",
        "Send TV power codes to turn off TVs. Region can be 'NA' or 'EU'.",
        PropertyList({
            Property("region", kPropertyTypeString, "NA")
        }),
        [](const PropertyList& properties) -> ReturnValue {
            auto region = properties["region"].value<std::string>();
            IrService::GetInstance().StartTvBGone(region);
            return true;
        });

    AddTool("self.ir.start_jammer",
        "Start IR jammer with specified mode (0: Basic, 1: Enhanced, 2: Sweep, 3: Random, 4: Empty).",
        PropertyList({
            Property("mode", kPropertyTypeInteger, 0)
        }),
        [](const PropertyList& properties) -> ReturnValue {
            auto mode = static_cast<IrJammerMode>(properties["mode"].value<int>());
            IrService::GetInstance().StartJammer(mode);
            return true;
        });

    AddTool("self.ir.stop",
        "Stop any current IR activity (TV-B-Gone or Jammer).",
        PropertyList(),
        [](const PropertyList& properties) -> ReturnValue {
    ...
    AddTool(
        ...
        [bad_usb](const PropertyList& properties) -> ReturnValue {
            ...
            std::string lang = properties["lang"].value<std::string>();
