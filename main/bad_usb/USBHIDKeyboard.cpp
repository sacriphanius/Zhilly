/*
 * USBHIDKeyboard.cpp
 * Bruce firmware'in kanıtlanmış USB HID düzeninden uyarlanan ESP-IDF implementasyonu.
 * - Saf HID klavye (CDC yok)
 * - Semaphore tabanlı güvenli rapor gönderimi
 * - Minimal, stabil TinyUSB yapılandırması
 */
...
// ── HID Report Descriptor (Sadece klavye, CDC yok) ──────────────────────────
...
static const tusb_desc_device_t device_descriptor = {
    ...
    .bcdUSB             = 0x0200,
    .bDeviceClass       = 0x00,          // Sınıf interface'te tanımlı
...
// ── Configuration Descriptor (Sadece HID IN endpoint) ───────────────────────
...
// Rapor tamamlandığında semaphore'u serbest bırak (Bruce'daki tud_hid_report_complete_cb ile aynı)
...
// ── begin(): TinyUSB sürücüsünü başlat ───────────────────────────────────────
...
// ── sendReport(): semaphore ile güvenli gönderim (Bruce mantığı) ──────────────
void USBHIDKeyboard::sendReport(KeyReport* keys) {
    ...
    // Önceki yarım kalan semaphore'u temizle
    ...
    if (res) {
        // Raporun gönderilmesini en fazla 100ms bekle
...
// ── press / release (Bruce ile birebir aynı mantık) ──────────────────────────
size_t USBHIDKeyboard::press(uint8_t k) {
    if (k >= 0x88) {
        ...
        if (k >= 128) return 0;   // ASCII sınır güvenliği
        k = _shiftCache[k];
        if (!k) return 0;
        if (k & SHIFT) {
