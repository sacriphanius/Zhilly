        ...
        if (bits & MAIN_EVENT_CLOCK_TICK) {
            ...
            if (screen_on_) {
                ...
                if (inactivity_seconds_ >= 180 && GetDeviceState() == kDeviceStateIdle) {
                    ESP_LOGI(TAG, "Inactivity timeout (3 min), dimming screen");
...
void Application::ResetInactivityTimer() {
    inactivity_seconds_ = 0;
    if (!screen_on_) {
        ESP_LOGI(TAG, "Activity detected, turning screen on");
        SetScreenDimming(false);
    }
}

void Application::SetScreenDimming(bool dim) {
    auto& board = Board::GetInstance();
